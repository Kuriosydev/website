  ...
  useEffect(() => {
    async function fetchPlaytimeContent() {
      try {
        ...
        const section =
          data.data[0]?.ParentsMetaData[0]?.playtimeSection?.[0] || {};

        const features =
          section.ParentsCard?.map((card) => ({
            heading: card.content_left_title,
            text: card.content_left_description,
            imgSource: "", // keep empty if not provided in CMS
          })) || [];
      ...
      <div className="w-full h-full relative overflow-hidden pb-14 md:py-10">
        ...
        <div className="flex flex-col sm:flex-col md:flex-col lg:flex-row xl:flex-row flex-wrap items-center justify-center px-4 sm:px-10 md:px-20 lg:px-20">
          ...
          <div className="w-full sm:w-full md:w-full lg:w-1/2 xl:w-1/2 flex flex-col items-center justify-center px-4 sm:px-10 md:px-20 lg:px-20">
            {playtime.features.map((feature, index) => (
              <Feature
                key={index}
                variant="left"
                ...
                otherClasses="my-3 md:my-4 w-full"
              />
            ))}
          </div>
        </div>
