...
export default function GameFeature() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch("https://cms.kurixel.com/api/cms-pages?filters[slug][$eq]=games&populate=GamesMetaData.GamePracticeSection.card.image&populate=GamesMetaData.GamePracticeSection.image");
  ...
  // Get video source from CMS or use fallback
  const videoSrc = item.image && item.image.length > 0
    ? `https://cms.kurixel.com${item.image[0].url}`
    : "/videos/news_banner_vid_2.mp4";

  // Create game features array with dynamic data from API
  const gameFeatures = item.card && item.card.length > 0
    ? item.card.map(card => ({
