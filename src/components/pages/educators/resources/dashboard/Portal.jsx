    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          "https://cms.kurixel.com/api/cms-pages?filters[slug][$eq]=educators-report&populate=EducatorsReportPage.list&populate=EducatorsReportPage.card"
        );

        const pageData = res.data?.data?.[0]?.EducatorsReportPage || [];

        const portalSection = pageData.find(
          (section) => section.__component === "shared.educators-report-portal-section"
        );

        if (portalSection) {
          const { card, list } = portalSection;
          setPortalData({
            title: card?.[0]?.title || "",
